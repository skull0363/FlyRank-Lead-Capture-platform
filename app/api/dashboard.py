...
def stats(tenant: Tenant = Depends(get_current_tenant), db: Session = Depends(get_db)):
    ...
    by_widget = (db.query(Submission.widget_id, func.count(Submission.id))
                 .filter(Submission.tenant_id == tenant.id)
                 .group_by(Submission.widget_id).all())
    by_country = (db.query(Submission.country, func.count(Submission.id))
                  .filter(Submission.tenant_id == tenant.id)
